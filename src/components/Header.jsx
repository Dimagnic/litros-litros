  ...
  return (
    <header style={{
      ...
      <div className="container" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', height:'4rem' }}>
        ...
        {/* Logo */}
        <button onClick={() => handleNav('/')} style={{ display:'flex', alignItems:'center', gap:'.6rem', background:'none', border:'none', cursor:'pointer' }}>
        ...
        <nav className="desk-nav" style={{ display:'flex', alignItems:'center', gap:'.1rem' }}>
          {NAV.map(n => (
            <button key={n.path} onClick={() => handleNav(n.path)} style={{
              padding:'.4rem .85rem', borderRadius:'.5rem',
              fontSize:'.82rem', fontWeight: isActive(n.path) ? 700 : 500,
              color: n.special ? 'var(--primary)' : isActive(n.path) ? 'var(--primary)' : 'rgba(242,242,242,.75)',
              background: isActive(n.path) && !n.special ? 'rgba(239,68,68,.1)' : 'transparent',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'.4rem' }}>
          ...
          {isAdmin && (
            <button onClick={signOut} style={{
              ...
              padding:'.32rem .6rem', fontSize:'.75rem', cursor:'pointer', fontFamily:'var(--font-body)',
            }}>Salir</button>
          )}
        </div>
      </div>
        ...
        <div style={{ padding:'1rem', borderTop:'1px solid var(--border)', background:'var(--card)', display:'flex', flexDirection:'column', gap:'.4rem' }}>
          {NAV.map(n => (
            <button key={n.path} onClick={() => handleNav(n.path)} style={{
              ...
              color: n.special ? 'var(--primary)' : isActive(n.path) ? 'var(--primary)' : 'var(--fg-muted)',
              background: isActive(n.path) && !n.special ? 'rgba(239,68,68,.1)' : 'transparent',
